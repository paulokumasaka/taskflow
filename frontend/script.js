...
async function fetchWithTimeout(url, options = {}, timeoutMs = TIMEOUT) {
  ...
  try {
    ...
    if (error.name === "AbortError") {
      throw new Error(
        "Request timeout. Make sure the backend is running on http://localhost:5000",
      );
    }
    throw error;
  }
...
if (registerForm) {
  registerForm.addEventListener("submit", async (e) => {
    ...
    // Validation
    if (!name) {
      showMessage("Por favor, digite seu nome.", "warning");
      return;
    }
    if (!isValidEmail(email)) {
      showMessage("Por favor, digite um endereço de email válido.", "warning");
      return;
    }
    if (!password || password.length < 3) {
      showMessage("A senha deve ter pelo menos 3 caracteres.", "warning");
      return;
    }
...
async function loadTasks() {
  if (!currentUserId) {
    if (alertMessage) {
      alertMessage.textContent =
        "Nenhum ID de usuário fornecido. Por favor, registre-se primeiro.";
  ...
  try {
    ...
    tasks.forEach((task) => {
      ...
      li.innerHTML = `
                ...
                <div class="task-actions">
                    <button class="btn btn-sm btn-outline-success complete-btn" title="Mark as ${task.status === "concluído" ? "pendente" : "concluído"}">
    ...
    if (alertMessage) {
      alertMessage.textContent = `Erro ao carregar tarefas: ${error.message}`;
      alertMessage.className = "alert alert-danger";
      alertMessage.style.display = "block";
    }
  }
  ...
  document.querySelectorAll(".complete-btn").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      try {
        ...
        if (!response.ok) {
          const data = await response.json();
          throw new Error(data.error || "Falha ao atualizar a tarefa");
        }
  ...
  document.querySelectorAll(".delete-btn").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      try {
        ...
        if (!response.ok) {
          const data = await response.json();
          throw new Error(data.error || "Falha ao excluir a tarefa");
        }
...
if (taskForm && taskList) {
  ...
  if (!currentUserId || isNaN(currentUserId) || currentUserId <= 0) {
    ...
    taskForm.addEventListener("submit", async (e) => {
      ...
      try {
        showMessage("Adicionando tarefa...", "info", "alertMessage");
        ...
        taskForm.reset();
        loadTasks();
        showMessage("Tarefa criada com sucesso!", "success", "alertMessage");
      } catch (error) {
        console.error("Erro ao criar tarefa:", error);
        showMessage(`Erro: ${error.message}`, "danger", "alertMessage");
      }
    });
  }
}
